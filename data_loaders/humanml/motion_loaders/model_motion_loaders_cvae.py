...
from data_loaders.humanml.motion_loaders.comp_v6_model_dataset_cvae import (
    CompCVAEGeneratedDataset,
)
...
def get_motion_loader(
    ...
    if "v6" in opt.name:
        dataset = CompV6GeneratedDataset(
            opt, ground_truth_dataset, w_vectorizer, mm_num_samples, mm_num_repeats
        )
    else:
        raise KeyError("Dataset not recognized!!")
...
def get_cvae_loader(
    ...
    dataset = CompCVAEGeneratedDataset(
        model,
        ground_truth_loader,
        mm_num_samples,
        mm_num_repeats,
